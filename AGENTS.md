...
*   **Model Response Parsing**: The `_parse_output` method in `agent.py` is responsible for strictly parsing AI responses, extracting thoughts (within `<think>...</think>`) and commands (JSON payloads), or plain text. Agents generating responses should adhere to this structure.
