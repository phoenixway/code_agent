...
def build_memory_content_with_action_commit_candidate(
    semantic_result: BoardCheckpointSemanticResult | None,
) -> MemoryCommitCandidate:
    """Builds a typed commit candidate for MEMORY_CONTENT_WITH_ACTION."""
    if not semantic_result:
        return MemoryCommitCandidate(blocking_reasons=("no_semantic_result",))
    ...
    # This is the key distinction from bare marker + action.
    # Durable content has memory tags, but not the specific checkpoint marker.
    is_content_with_action = (
        is_mca and semantic_result.compiler_has_memory_tags and not semantic_result.compiler_has_memory_checkpoint
    )

    if not is_content_with_action:
        return MemoryCommitCandidate(blocking_reasons=("not_clean_memory_content_with_action",))

    return MemoryCommitCandidate(
        branch="MEMORY_CONTENT_WITH_ACTION",
        candidate_available=True,
        checkpoint_kind=semantic_result.kind.name,
        ...
        expected_commit_attempted=True,
        expected_commit_accepted_count=1,
        expected_commit_rejected_count=0,
        expected_last_memory_update_done=False,
        expected_pass_through_preserved=True,
        blocking_reasons=(),
    )


def resolve_memory_content_with_action_commit_authority(
    semantic_result: BoardCheckpointSemanticResult | None,
    legacy_branch: str,
    legacy_handled: bool,
    legacy_reason: str,
    ...
    """Resolves commit authority for the MEMORY_CONTENT_WITH_ACTION branch."""
    candidate = build_memory_content_with_action_commit_candidate(semantic_result)

    handled_agreement = candidate.expected_handled == legacy_handled if candidate.candidate_available else False
    is_pass_through_mca = bool(
        candidate.candidate_available
        and candidate.branch == "MEMORY_CONTENT_WITH_ACTION"
        and candidate.expected_handled is False
        and legacy_handled is False
    )
    reason_agreement = (
        True
        if is_pass_through_mca
    ...
    # For content+action, there is no <memory_update_done /> marker to remove.
    checkpoint_removed_agreement = not legacy_checkpoint_removed if candidate.candidate_available else False
    branch_agreement = legacy_branch == "MEMORY_CHECKPOINT_WITH_ACTION" if candidate.candidate_available else False
    ...
    diagnostic = MemoryCommitAuthorityDiagnostic(
        branch="board_memory.memory_content_with_action",
        switch_value=switch_value,
        authority_source=authority_source,
        effective_source="legacy",
        selected_by_switch=selected_by_switch,
