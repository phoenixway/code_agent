...
class TestCLICommands(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        # Mock the HistoryInput widget (now a TextArea) and the custom Submitted message
        self.mock_sender = MagicMock()
        self.mock_sender.text = ""
        self.message = MagicMock()
        self.message.sender = self.mock_sender

    async def test_add_command_single_file(self):
        """Test /add with a single file."""
        self.message.text = "/add test.py"
        self.agent.context_manager.add_path.return_value = 1
        
        with patch.object(self.app, 'run_worker') as mock_run_worker:
            await self.app.on_history_input_submitted(self.message)
            # Command handlers are now run in a worker
            self.assertTrue(mock_run_worker.called)
            # To test the logic, we can await the coroutine passed to the worker
            await mock_run_worker.call_args[0][0]
    ...
    async def test_add_command_multiple_files(self):
        ...
        self.message.text = '/add file1.py "folder with spaces/file2.py"'
    ...
    async def test_drop_command_specific_files(self):
        ...
        self.message.text = "/drop file1.py file2.py"
    ...
    async def test_cd_command(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                self.message.text = f"/cd {tmpdir}"
                with patch.object(self.app, 'run_worker') as mock_run_worker:
                    await self.app.on_history_input_submitted(self.message)
                    await mock_run_worker.call_args[0][0]
                
                # The os.chdir is called within the test's process, so this check is valid
