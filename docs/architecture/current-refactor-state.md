...
- **Phase**: Phase 59 — Tool-Local `edit_file` Recovery Policy Closure
- **Status**: Complete.
- **Next Step**: Phase 60 — Next Semantic Runtime Slice Selection.
- **Boundary**: Closed the follow-up recovery hardening after Phase 57. Phase 58 made repeat-action tripwires turn-scoped by clearing only `consecutive_same_action_count`, `last_completed_fingerprint`, and `forbidden_next_action_fingerprint` at real user-input boundaries, without clearing active intent, working material, failed-action diagnostics, plan-review state, or operational journal state. Phase 59 centralized `edit_file` recovery rails next to the tool layer in `modules/tools/recovery/edit_file_recovery_policy.py`. `EditFileTool` search mismatches, malformed `edit_file` schema preflight, and dispatcher repeated-`edit_file` hard-stop now use the same tool-local recovery policy. `replace_symbol` is now known to intent runtime and preserved in MODIFY/default/reuse contracts, so `extract_symbol` → `replace_symbol` is no longer advertised by recovery while being stripped from the active intent contract. MODIFY recovery now prefers structural `read_chunk`/`extract_symbol`/`replace_symbol`/`edit_file`/`write_file_block` rails where appropriate, while non-MODIFY recovery remains read-only. No global defect-detector disablement, ActionPolicy authority change, plan-review gate behavior change, automatic intent completion, authority transfer, broad read/rewrite expansion, unsupported-language expansion for `replace_symbol`, or legacy cleanup was added.
