...
| **`IntentTransitionHandler` (plaintext completion)** | `sanitize_visible_text_for_user` (regex) | Final-answer/plaintext guard | `accessors.get_visible_text` | Medium | Phase 8 (Inventory Complete) |
...
| **`ResponsePipelinePrevalidationMixin._reject_truncated_terminal_completion_before_transition`** | `step.intent_payload`, `terminal_plaintext_completion_status(raw_response)` | Final-answer/plaintext guard | N/A / later terminal-answer validator | Medium | Phase 8 (Inventory Complete) |
| **`ResponsePipelineStagesMixin` (leaked system result check)** | `ResponseSemantics.looks_like_leaked_system_result` | Final-answer/plaintext guard | `accessors.is_leaked_system_result` | Medium | Migrated |
...
| **`DispatchOutcomeHandler._extract_visible_text`** | `extract_visible_text_for_user` (regex) | Final-answer/plaintext guard | `accessors.get_visible_text` | Medium | Phase 8 (Inventory Complete) |
| **`DispatchOutcomeHandler._strip_leaked_system_results_from_ui_text`** | Regex on raw response | Final-answer/plaintext guard | `accessors.get_visible_text` | Medium | Phase 8 (Inventory Complete) |
