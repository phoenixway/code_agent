...
| **`ResponsePipelinePrevalidationMixin._reject_compiler_invalid_atomic_bundle_before_transition`** | `compiler_error_code`, `invalid_kind` from `parsed_output` | Atomic bundle validation / dispatch authority boundary (compiler path) | N/A / later dedicated bundle validator | High | Phase 6 (Design Approved) |
