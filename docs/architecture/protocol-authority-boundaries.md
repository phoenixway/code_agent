...
|---|---|---|---|---|---|
| **`ResponseSemantics.has_any_action_proposal`** | `parsed_output.has_action_segment`, `parsed_action_count`, `compiler_ir.action_ops` | Compatibility action proposal, recovery evidence | `accessors.has_any_action_proposal_compat` | High | Migrated |
...
| **`ModelOutputRecoveryHandler._has_any_action_proposal`** | `ResponseSemantics.has_any_action_proposal` | Compatibility action proposal | `accessors.has_any_action_proposal_compat` | Medium | Migrated |
...
| **`TransitionFollowupSemantics`** | `ProtocolCompiler.analyze()` on fragments | Intent transition/followup policy | `accessors.get_followup_surface` | High | Phase 5 Complete (with fallbacks) |
| **`IntentTransitionHandler` followup helpers** | `TransitionFollowupSemantics`, regex, `TransitionSemanticValidator` | Intent transition/followup policy | `accessors.get_followup_surface` | High | Phase 5 Complete (with fallbacks) |
| **`IntentTransitionHandler` (plaintext completion)** | `sanitize_visible_text_for_user` (regex) | Final-answer/plaintext guard | `accessors.get_visible_text` | Medium | Phase 8 (Step 4A Done) |
...
| **`ResponsePipelinePrevalidationMixin._reject_compiler_invalid_atomic_bundle_before_transition`** | `BundleSemanticValidator` | Atomic bundle validation / dispatch authority boundary (compiler path) | N/A / later dedicated bundle validator | High | Phase 6 (Migrated) |
| **`ResponsePipelinePrevalidationMixin._reject_invalid_atomic_bundle_before_transition`** | `compiler_error_code`, `invalid_kind`, `compiler_ir`, `segments`, `ActionPolicyHandler.validate_atomic_bundle_action` | Atomic bundle validation / dispatch authority boundary | N/A / later dedicated bundle validator | High | Phase 7 (Complete) |
| **`ResponsePipelinePrevalidationMixin._reject_truncated_terminal_completion_before_transition`** | `step.intent_payload`, `terminal_plaintext_completion_status(raw_response)` | Final-answer/plaintext guard | N/A / later terminal-answer validator | Medium | Phase 8 (Step 4A Done) |
| **`ResponsePipelineStagesMixin` (leaked system result check)** | `typed TerminalAnswerClassifier result + accessors.is_leaked_system_result fallback` | Final-answer/plaintext guard | `TerminalAnswerClassifier` (typed result primary signal plus legacy fallback; not sole authority) | Low | Phase 8 Step 4L complete |
...
| **`DispatchOutcomeHandler._extract_visible_text`** | `extract_visible_text_for_user` (regex) | Final-answer/plaintext guard | `accessors.get_visible_text` | Medium | Phase 8 (Step 4A Done) |
| **`DispatchOutcomeHandler._strip_leaked_system_results_from_ui_text`** | Regex on raw response | Final-answer/plaintext guard | `accessors.get_visible_text` | Medium | Phase 8 (Step 4A Done) |
