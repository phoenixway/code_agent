...
| **`TransitionFollowupSemantics`** | `ProtocolCompiler.analyze()` on fragments | Intent transition/followup policy | `accessors.get_followup_surface` | High | Phase 5 Design Approved |
| **`IntentTransitionHandler` followup helpers** | `TransitionFollowupSemantics`, regex | Intent transition/followup policy | `accessors.get_followup_surface` | High | Phase 5 Design Approved |
