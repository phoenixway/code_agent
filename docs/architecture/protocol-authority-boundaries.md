...
| **`ModelOutputRecoveryHandler._has_any_action_proposal`** | `ResponseSemantics.has_any_action_proposal` | Compatibility action proposal | `accessors.has_any_action_proposal_compat` | Medium | Plan Proposed |
...
| **`runtime_protocol_semantics.output_recovery_compiler_metadata`** | `RuntimeProtocolSemantics` or `parsed_output` | Compiler metadata | `accessors.get_compiler_metadata` | Low | Plan Proposed |
