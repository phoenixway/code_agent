...
| **`ResponseGuardPolicy.is_nonproductive_thinking_turn`** | `ResponseSemantics` helpers | Runtime policy (loop detection) | `accessors.has_substantial_think`, `accessors.has_any_action_proposal` | Medium | Migration Design Proposed |
...
| **`ResponsePipelineStagesMixin` (leaked system result check)** | `ResponseSemantics.looks_like_leaked_system_result` | Final-answer/plaintext guard | `accessors.is_leaked_system_result` | Medium | Migration Design Proposed |
